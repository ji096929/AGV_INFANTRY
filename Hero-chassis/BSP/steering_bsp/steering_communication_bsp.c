...
uint8_t steering_communication_transmit(steering_communication_ctx_t *ctx, steering_communication_pack_t *pack)
{
    ...
    memcpy((uint8_t*)&Extid + sizeof(pack->steering_id), &pack->data2, sizeof(pack->data2));
    memcpy((uint8_t*)&Extid + sizeof(pack->steering_id) + sizeof(pack->data2), &pack->cmd_id, sizeof(pack->cmd_id));
    // 调用发送函数
