...
void DMA2_Stream1_IRQHandler(void)
{
	...
	memset(JudgeReceiveBuffer,0,JudgeBufBiggestSize);
	HAL_UART_Receive_DMA(&huart6,JudgeReceiveBuffer,45);
}

void JudgeBuffReceive(unsigned char ReceiveBuffer[],uint16_t DataLen)
{
	JudgeReceive.last_receive_time=JudgeReceive.receive_time;
	uint16_t cmd_id;
	short PackPoint;
	...
	JudgeReceive.receive_time	= time.total_count;
	}
}