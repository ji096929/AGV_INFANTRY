...
int16_t Difference_Filter(int16_t input_data,int16_t error_data[])
{
//	int16_t valid=1;
//    if((input_data-error_data[1]>DIFF_MAX)||(error_data[1]-input_data>DIFF_MAX))
//    {
//        valid=0;
//    }

//    if((input_data<SAMPLE_MIX)||(input_data>SAMPLE_MAX))
//    {
//        valid=0;
//    }
...
//    if(valid==0)
//    {
//        input_data=2*error_data[1]-error_data[0];
//        if(input_data<SAMPLE_MIX)
//        {
//            input_data=SAMPLE_MIX;
//        }
//        else if(input_data>SAMPLE_MAX)
//        {
//            input_data=SAMPLE_MAX;
//        }
//    }
		int16_t output_data;
    error_data[0]=error_data[1];
		error_data[1]=error_data[2];
		error_data[2]=error_data[3];
		error_data[3]=error_data[4];
		error_data[4]=error_data[5];
		error_data[5]=error_data[6];
		error_data[6]=error_data[7];
		error_data[7]=error_data[8];
		error_data[8]=error_data[9];
    error_data[9]=input_data;
		output_data=(error_data[0]+error_data[1]+error_data[2]+error_data[3]+error_data[4]+error_data[5]+error_data[6]+error_data[7]+error_data[8]+error_data[9])/10;
    return output_data;
}
...
void M3508_Init(M3508_T *motor,uint16_t ID,float reduction_rate, float torque_coefficient)
{
...
/*******************************GM6020 Part********************************************/
 ...
 void GM6020_Init(GM6020_T *motor,uint16_t ID,float torque_coefficient)
 {
    motor->parameter.ID =   ID;
    motor->parameter.torque_coefficient =   torque_coefficient;
    motor->parameter.calibrate_state    =   MOTOR_NO_CALIBRATE;
...
//		 motor->status.velocity_rpm =   motor->feedback.velocity_lsb;
    ...
    motor->status.last_total_position_degree =   motor->status.total_position_degree;
		if(motor->parameter.calibrate_state==   MOTOR_NO_CALIBRATE)
    {
    ...
    motor->status.total_position_rad    =   motor->status.position_rounds*2.0f*PI+motor->status.position_rad;
        ...
        motor->status.zero_position_rad    =   motor->status.total_position_rad;
				...
				if(motor->parameter.calibrate_state==   MOTOR_CALIBRATED)
    {
